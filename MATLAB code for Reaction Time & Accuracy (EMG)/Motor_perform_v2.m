...
for iFile = 1:length(a)
    ...
    data = load_acq(FileName);


    %% Align grip with timing
    data_ds = downsample(data.data,10);
    fs = 250;
...
[h.etacs,p.etacs]=ttest2(error{4},error{5});
[h.e15min,p.e15min]=ttest2(error{4},error{1});
[h.e45min,p.e45min]=ttest2(error{4},error{2});
[h.e75min,p.e75min]=ttest2(error{4},error{3});

[h.rtacs,p.rtacs]=ttest2(RT{4},RT{5});
[h.r15min,p.r15min]=ttest2(RT{4},RT{1});
[h.r45min,p.r45min]=ttest2(RT{4},RT{2});
[h.r75min,p.r75min]=ttest2(RT{4},RT{3});
